...
from .log_gatherer_annotator import cleanup_ctx_file, ensure_run_paths
...
def _init_paths(config: dict | None = None):
    """Initialize file paths for the current analysis run."""
    global CTX_FILE_PATH, VERBOSE_LOG_PATH
    CTX_FILE_PATH, VERBOSE_LOG_PATH = ensure_run_paths(config)

# --- Annotator Logic ---

# Helper function to get text content from MCP tool (similar to plugin.py)
async def _mcp_get_tool_text_content(session: ClientSession, tool_name: str, params: dict | None = None) -> str | None:
    try:
...
def call_openai_llm_annotator(system_prompt: str, user_prompt: str, api_key: str, model: str, base_url: str, system_prompt_at_bottom: bool, prompt_token_warning: int, max_tokens: int = 8192, extra_body: dict = None, custom_ca_cert_path: str = "", client_cert_path: str = "", client_key_path: str = "") -> str:
    try:
        ...
        os.environ["TIKTOKEN_CACHE_DIR"] = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + r"\encodings"
        ...
        if len(enc.encode(user_prompt + system_prompt)) > prompt_token_warning:
            print("[AETHER] [Annotator] Pseudo Code sent may be cut due to being too long.")
    except Exception as e:
...
async def mcp_execute_tool(session: ClientSession, tool_name: str, params: dict | None = None) -> bool:
    try:
...
def extract_root_function_name(ctx_content: str) -> str | None:
...
async def run_annotator_agent(config: dict):
    _init_paths(config)
    def _cleanup_ctx() -> None:
        cleanup_ctx_file(config)
    ...
    try:
        with open(CTX_FILE_PATH, encoding="utf-8") as f:
            ctx_content = f.read()
