...
class FunctionIndexManager:
    ...
    def get_index() -> FunctionIndex:
        ...
        if not _legacy_migrated:
            try:
                FunctionIndexManager.migrate_legacy_indexes()
            finally:
    ...
    @staticmethod
    def migrate_legacy_indexes() -> int:
        """Move legacy flat index files into per-binary folders.

        Returns the number of files migrated.
        """
        base_dir = get_index_base_dir()
        try:
        ...
        for name in entries:
            ...
            target_path = get_index_filepath(identifier)
            ...
            try:
                ...
                print(f"[AETHER] Failed to migrate legacy index {legacy_path}: {exc}")

        if migrated:
            print(f"[AETHER] Migrated {migrated} legacy index file(s)")
        return migrated
    ...
    def clear_index_completely() -> bool:
        ...
        legacy_path = get_legacy_index_filepath(identifier)
        if os.path.isfile(filepath):
            try:
                ...
                try:
                    if os.path.isdir(index_dir) and not os.listdir(index_dir):
                        os.rmdir(index_dir)
                except OSError:
                    pass
                return True
        ...
        if os.path.isfile(legacy_path):
            try:
                ...
                print(f"[AETHER] Deleted legacy index file: {legacy_path}")
                return True
            except OSError as e:
                print(f"[AETHER] Failed to delete legacy index file: {e}")
                return False
        return True
