...
class CustomPromptReAnnotateHandler(ida_kernwin.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        if dlg.exec_():
            ...
            if start_pipeline(custom_reannotate_thread(aire_functions, config, current_func_name, current_func_addr, user_advice)) is False:
                print("[AETHER] [Custom Re-annotate] Error running custom re-annotation: Another function is currently being executed")
        return 1
...
class StripAIAnnotationsHandler(ida_kernwin.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        if start_pipeline(strip_thread(aire_functions, config, current_func_name)) is False:
            print("[AETHER] [Strip Only] Error during annotation stripping: Another function is currently being executed")
        return 1
