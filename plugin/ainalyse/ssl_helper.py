...
def create_openai_client_with_custom_ca(api_key: str, base_url: str, custom_ca_cert_path: str = "", client_cert_path: str = "", client_key_path: str = "", feature: str = ""):
    ...
    version = get_version()
    if feature:
        custom_headers = { "User-Agent": f"AETHER (IDA)/{version}-{feature}"}
    else:
        custom_headers = { "User-Agent": f"AETHER (IDA)/{version}"}
...
def create_async_openai_client_with_custom_ca(api_key: str, base_url: str, custom_ca_cert_path: str = "", client_cert_path: str = "", client_key_path: str = "", feature: str = ""):
    """Create AsyncOpenAI client and underlying AsyncClient for async workflows.

    Returns:
        tuple: (AsyncOpenAI client, httpx.AsyncClient transport)
    """
    ...
    # Create async httpx client with SSL verification disabled
    try:
        if client_cert_key:
            httpx_client = httpx.AsyncClient(cert=client_cert_key, verify=False, timeout=600)
        else:
            httpx_client = httpx.AsyncClient(verify=False, timeout=600)
        print("[AETHER] SSL verification disabled for Async HTTPX client")
    except Exception as e:
        print(f"[AETHER] Warning: Error creating async httpx client: {e}. Using basic async client.")
        httpx_client = httpx.AsyncClient(verify=False, timeout=600)
