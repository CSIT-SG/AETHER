...
# --- File Paths ---
PROMPT_GATHERER = os.path.join(os.path.dirname(__file__), "prompts/struct-gatherer-prompt.txt")
...
# --- Gatherer Logic ---
class Node:
    def __init__(self, name: str, address: Optional[str] = None, parent_name: Optional[str] = None, struct_name: Optional[str] = None):
        self.name = name
...
class GatheringResultsDialog(QtWidgets.QDialog):
    ...
    def __init__(self, call_tree_str, pseudocode_store, parent=None):
        ...
        # Header
        header_label = QtWidgets.QLabel("LLM Struct Gatherer has completed. Review the results and choose your next action:")
...
async def run_gatherer_agent(config: dict,variable_name: str):
        ...
        try:
            async with sse_client(server_url) as streams:
                async with ClientSession(streams[0], streams[1]) as session:
                    ...
                    print(f"[AETHER] Tracking Variable: {variable_name}")
                    ...
                    for i in range(max_iterations):
                        ...
                        current_prompt_content = current_prompt_content.replace("{VARIABLE}", variable_name)
