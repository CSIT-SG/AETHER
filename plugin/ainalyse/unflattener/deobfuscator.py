...
def update_display_error(error: str, func_addr):
    viewer_instance = g_ai_deobfs_viewers.get(AI_DEOBFS_VIEW_TITLE + " " + str(func_addr))
    if viewer_instance:
    ...
    try:
        ...
        if debug:
            with open(f"{task}_{current_time}.txt", "a", encoding='utf-8') as outputfile:
                outputfile.write(response.choices[0].message.content.strip() or "No response from LLM")
...
async def run_deobfuscator(config: dict, current_func_name: str, current_func_addr: str) -> bool:
    ...
    unflatten_attempts = config.get("UNFLATTEN_ATTEMPTS", 3)

    tab_name = AI_DEOBFS_VIEW_TITLE + " " + str(current_func_addr)
    viewer_instance = g_ai_deobfs_viewers.get(tab_name)
    if viewer_instance:
        viewer_instance.SetGenerating(True)
        viewer_instance.UpdateDisplay()
        viewer_instance.Show(tab_name)
    ...
    if not mcp_success:
        ...
        update_display_error(f"[AInalyse] [AI Unflatten] MCP connection failed: {mcp_msg}", current_func_addr)
        return False
    ...
    try:
        ...
        update_display_error(f"[AInalyse] [AI Unflatten] Error: Unflatten Prompt file not found at {DEOBFUSCATOR_PROMPT_FILE}", current_func_addr)
        return False
    try:
        ...
        update_display_error(f"[AInalyse] [AI Unflatten] Error: Check Prompt file not found at {DEOBFUSCATOR_CHECK_FILE}", current_func_addr)
        return False
    try:
        ...
        update_display_error(f"[AInalyse] [AI Unflatten] Error: Label Prompt file not found at {DEOBFUSCATOR_LABEL_FILE}", current_func_addr)
        return False
    ...
    try:
        async with sse_client(server_url) as streams:
            async with ClientSession(streams[0], streams[1]) as session:
                ...
                for i in range(unflatten_attempts):
                    ...
                    if not llm_response:
                        ...
                        update_display_error("No response from LLM.", current_func_addr)
                        return False
                    ...
                    if "Deobfuscated code is not fully equivalent to obfuscated code" in llm_response and  i < unflatten_attempts - 1:
                        ...
                        if "Deobfuscated code is not fully equivalent to obfuscated code" in llm_response:
                            print("[AInalyse] [AI Unflatten] Deobfuscated code doesn't match obfuscated code...")
                            print("[AInalyse] [AI Unflatten] Unflattening attempts has exceeded maximum attempts set. Displaying latest code generated. Code displayed may be inaccurate...")
        ...
        import traceback
        traceback.print_exc()
        update_display_error(f"{e}", current_func_addr)
        return False
