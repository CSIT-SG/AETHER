...
class AIDeobfsScrollHooks(ida_hexrays.Hexrays_Hooks):
    ...
    def __init__(self, func_addr):
        super().__init__()  # initialize Hexrays_Hooks
    ...
    def curpos(self, vdui):
        ...
        form_instance = g_ai_deobfs_viewers.get(AI_DEOBFS_VIEW_TITLE + " " + self.func_addr)
...
class AIDeobfsForm(ida_kernwin.PluginForm):
    ...
    def __init__(self):
        ...
        self.current_func_addr = None
        self.scroll_hooks = None
    
    def create_scroll_hooks(self):
        # Install scroll hooks if not already installed
        if not self.scroll_hooks and ida_hexrays.init_hexrays_plugin():
            self.scroll_hooks = AIDeobfsScrollHooks(self.current_func_addr)
            self.scroll_hooks.hook()
            print("[AInalyse] [AI Unflatten] Installed scroll sync hooks")

    def remove_scroll_hooks(self):
        """Remove scroll hooks when plugin is terminated."""
        if self.scroll_hooks:
            self.scroll_hooks.unhook()
            self.scroll_hooks = None
    ...
    def OnCreate(self, form):
        ...
        g_ai_deobfs_viewers[AI_DEOBFS_VIEW_TITLE  + " " + str(self.current_func_addr)] = self
    ...
    def OnClose(self, form):
        ...
        if AI_DEOBFS_VIEW_TITLE + " " + str(self.current_func_addr) in g_ai_deobfs_viewers:
            del g_ai_deobfs_viewers[AI_DEOBFS_VIEW_TITLE  + " " + str(self.current_func_addr)]
        self.remove_scroll_hooks()
...
def show_or_update_ai_deobfs_tab(func_addr: str):
    ...
    def _show_update_sync():
        try:
            widget = ida_kernwin.find_widget(AI_DEOBFS_VIEW_TITLE + " " + str(func_addr))
            ...
            if widget:
                ...
                if new_form.Show(AI_DEOBFS_VIEW_TITLE + " " + str(func_addr)):
                    new_form.SetFunctionAddr(func_addr)
                else:
